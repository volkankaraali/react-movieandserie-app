...
function MostPopularSeries() {
    ...
    const { mostPopularSeries } = useMovie()
    const [pageNumber, setPageNumber] = useState(0)

    const seriesPerPage = 10;
    const pageVisited = pageNumber * seriesPerPage;

    const displaySerie = mostPopularSeries.slice(pageVisited, pageVisited + seriesPerPage).map((serie) => (
        <div className="max-w-xs rounded bg-white overflow-hidden shadow-md mx-auto">
            ...
            <div className="px-6 py-4">
                ...
                <Link to={`/detail/${serie.id}`} className="no-underline">
    ...
    const pageCount = Math.ceil(mostPopularSeries.length / seriesPerPage);
    ...
    return (
        <>
            <h3 className="text-center mt-10">Most Popular Series</h3>
